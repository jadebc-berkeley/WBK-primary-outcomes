...
# Midline
#----------------------------------------------
P.t1.df=coin.prep(m,tx="Passive Control",cont="Control",y="stunted")
set.seed(67890)
...
W.t1.df=coin.prep(m,tx="Water",cont="Control",y="stunted")
set.seed(67890)
W.t1.permtest=wilcoxsign_test(stunted~tr | block, data=W.t1.df,
   distribution=approximate(nresample=100000))
W.t1.perm.p=coin::pvalue(W.t1.permtest)

WSH.S.t1.df=coin.prep(m,tx="Sanitation",cont="Control",y="stunted")
set.seed(67890)
WSH.S.t1.permtest=wilcoxsign_test(stunted~tr | block, data=WSH.S.t1.df,
   distribution=approximate(nresample=100000))
WSH.S.t1.perm.p=coin::pvalue(WSH.S.t1.permtest)

H.t1.df=coin.prep(m,tx="Handwashing",cont="Control",y="stunted")
set.seed(67890)
H.t1.permtest=wilcoxsign_test(stunted~tr | block, data=H.t1.df,
   distribution=approximate(nresample=100000))
H.t1.perm.p=coin::pvalue(H.t1.permtest)

WSH.t1.df=coin.prep(m,tx="WSH",cont="Control",y="stunted")
set.seed(67890)
WSH.t1.permtest=wilcoxsign_test(stunted~tr | block, data=WSH.t1.df,
   distribution=approximate(nresample=100000))
WSH.t1.perm.p=coin::pvalue(WSH.t1.permtest)

N.t1.df=coin.prep(m,tx="Nutrition",cont="Control",y="stunted")
set.seed(67890)
N.t1.permtest=wilcoxsign_test(stunted~tr | block, data=N.t1.df,
   distribution=approximate(nresample=100000))
N.t1.perm.p=coin::pvalue(N.t1.permtest)

WSHN.t1.df=coin.prep(m,tx="Nutrition + WSH",cont="Control",y="stunted")
set.seed(67890)
WSHN.t1.permtest=wilcoxsign_test(stunted~tr | block, data=WSHN.t1.df,
   distribution=approximate(nresample=100000))
WSHN.t1.perm.p=coin::pvalue(WSHN.t1.permtest)

stunt_t1_h1_pval_unadj_j=data.frame(perm.pvalue=c(P.t1.perm.p,W.t1.perm.p,WSH.S.t1.perm.p,H.t1.perm.p,
                         WSH.t1.perm.p,N.t1.perm.p,WSHN.t1.perm.p))
rownames(stunt_t1_h1_pval_unadj_j)=c("Passive Control vs. C","Water vs. C",
...
# Midline
#----------------------------------------------
WSHN.N.t1.df=coin.prep(m,tx="Nutrition + WSH",cont="Nutrition",y="stunted")
set.seed(67890)
WSHN.N.t1.permtest=wilcoxsign_test(stunted~tr | block, data=WSHN.N.t1.df,
   distribution=approximate(nresample=100000))
WSHN.N.t1.perm.p=coin::pvalue(WSHN.N.t1.permtest)

WSHN.WSH.t1.df=coin.prep(m,tx="Nutrition + WSH",cont="WSH",y="stunted")
set.seed(67890)
...
stunt_t1_h3_pval_unadj_j=data.frame(perm.pvalue=c(WSHN.N.t1.perm.p,WSHN.WSH.t1.perm.p))
rownames(stunt_t1_h3_pval_unadj_j)=c("Nutrition + WSH vs. Nutrition",
...
# Endline
#----------------------------------------------
WSHN.N.t2.df=coin.prep(e,tx="Nutrition + WSH",cont="Nutrition",y="stunted")
set.seed(67890)
WSHN.N.t2.permtest=wilcoxsign_test(stunted~tr | block, data=WSHN.N.t2.df,
   distribution=approximate(nresample=100000))
WSHN.N.t2.perm.p=coin::pvalue(WSHN.N.t2.permtest)

WSHN.WSH.t2.df=coin.prep(e,tx="Nutrition + WSH",cont="WSH",y="stunted")
set.seed(67890)
...
stunt_t2_h3_pval_unadj_j=data.frame(perm.pvalue=c(WSHN.N.t2.perm.p,WSHN.WSH.t2.perm.p))
rownames(stunt_t2_h3_pval_unadj_j)=c("Nutrition + WSH vs. Nutrition",
                                 "Nutrition + WSH vs. WSH")


save(stunt_t1_h1_pval_unadj_j,
     stunt_t2_h1_pval_unadj_j,
     stunt_t1_h3_pval_unadj_j,
     stunt_t2_h3_pval_unadj_j,
     file=here("primary/res_data/stunt_pval_unadj.RData"))


stunt_t1_h1_pval_unadj_j
stunt_t1_h3_pval_unadj_j
