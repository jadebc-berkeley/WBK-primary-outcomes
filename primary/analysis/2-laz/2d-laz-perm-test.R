...
#----------------------------------------------
# midline
WSHN.N.t1.df=coin.prep(m,tx="Nutrition + WSH",cont="Nutrition",y="haz")
set.seed(67890)
WSHN.N.t1.permtest=wilcoxsign_test(haz~tr | block, data=WSHN.N.t1.df,
      distribution=approximate(B=100000))
WSHN.N.t1.perm.p=pvalue(WSHN.N.t1.permtest)

WSHN.WSH.t1.df=coin.prep(m,tx="Nutrition + WSH",cont="WSH",y="haz")
set.seed(67890)
WSHN.WSH.t1.permtest=wilcoxsign_test(haz~tr | block, data=WSHN.WSH.t1.df,
      distribution=approximate(B=100000))
WSHN.WSH.t1.perm.p=pvalue(WSHN.WSH.t1.permtest)

laz_t1_h3_pval_unadj_j=data.frame(perm.pvalue=c(WSHN.N.t1.perm.p,WSHN.WSH.t1.perm.p))
rownames(laz_t1_h3_pval_unadj_j)=c("Nutrition + WSH vs. Nutrition",
                                   "Nutrition + WSH vs. WSH")


# endline
WSHN.N.t2.df=coin.prep(e,tx="Nutrition + WSH",cont="Nutrition",y="haz")
set.seed(67890)
WSHN.N.t2.permtest=wilcoxsign_test(haz~tr | block, data=WSHN.N.t2.df,
      distribution=approximate(B=100000))
WSHN.N.t2.perm.p=pvalue(WSHN.N.t2.permtest)

WSHN.WSH.t2.df=coin.prep(e,tx="Nutrition + WSH",cont="WSH",y="haz")
set.seed(67890)
WSHN.WSH.t2.permtest=wilcoxsign_test(haz~tr | block, data=WSHN.WSH.t2.df,
      distribution=approximate(B=100000))
WSHN.WSH.t2.perm.p=pvalue(WSHN.WSH.t2.permtest)

laz_t2_h3_pval_unadj_j=data.frame(perm.pvalue=c(WSHN.N.t2.perm.p,WSHN.WSH.t2.perm.p))
rownames(laz_t2_h3_pval_unadj_j)=c("Nutrition + WSH vs. Nutrition","Nutrition + WSH vs. WSH")


save(laz_t1_h1_pval_unadj_j,laz_t1_h3_pval_unadj_j,
     file="~/Dropbox/WBK-primary-analysis/results/jade/laz_t1_pval_unadj.RData")

save(laz_t2_h1_pval_unadj_j,laz_t2_h3_pval_unadj_j,
     file="~/Dropbox/WBK-primary-analysis/results/jade/laz_t2_pval_unadj.RData")

laz_t1_h1_pval_unadj_j
laz_t1_h3_pval_unadj_j
laz_t2_h1_pval_unadj_j
laz_t2_h3_pval_unadj_j
