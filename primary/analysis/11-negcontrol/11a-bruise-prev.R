...
#----------------------------------------------
# N.0=table(d$tr[!is.na(d$bruise7)& d$time==0])
# n.0=table(d$bruise7[d$time==0],d$tr[d$time==0])[2,]
# 
# bruise_t0_n_j=data.frame(cbind(N.0=N.0,n.0=n.0))
...
#----------------------------------------------
# bruise_t0_prev_j=t(sapply(levels(d$tr), function(x) washb_mean(
#   Y=d$bruise7[d$tr==x & d$time==0],id=d$clusterid[d$tr==x & d$time==0],print=FALSE)))
...
# colnames(bruise_t0_prev_j)=c("N","Prev","SD","Robust SE","lb","ub")
...
# bruise_t0_prev_j=bruise_t0_prev_j[,c("Prev","lb","ub")]
