capture log close
log using "~/documents/crg/wash-benefits/Kenya/src/primary/dm/2_make_anthro_data_blind.log", replace
...
use "Baseline/msP_bl_append_ID_clean_20161010.dta"
...
* NEED INPUT on how to code the "other" category
gen walls=0
...
* ---------------------------------------------
* midline
use "Midline/msP_ml_append_ID_clean_anthropometry_20160909.dta", clear
...
* endline
preserve
use "Endline/msP_el_append_ID_clean_anthropometry_20160909.dta", clear
tostring hhid, replace
ren c422 c422e
ren c423 c423e
ren c424 c424e
...
* Child length
* ---------------------------------------------
use "Midline/msP_ml_child_anthro_20160909.dta", clear

* merge in mother anthro data
merge m:1 hhid using "Midline/msP_ml_append_ID_clean_anthropometry_20160909.dta"
...
* check for children whose ages might be incorrect
*list hhid childid if agem>14 

* replacing age as missing if age > 14 months
* replace agem_midline=. if agem_midline>14
...
* compare who and zscore06 results

/*foreach var of varlist haz waz whz {
	gen diff=`var'- `var'_who
	sum diff
	drop diff
}*/
...
* Child length
* ---------------------------------------------
use "Endline/msP_el_child_anthro_20160909.dta", clear

* merge in mother anthro data
merge m:1 hhid using "Endline/msP_el_append_ID_clean_anthropometry_20160909.dta"
* drop if there is no mother anthro data
drop if _m==2
drop _m
...
* check for children whose ages might be incorrect
*list dataid childid if age_endline>26 CHECK

* replacing age as missing if age > 26 months
*replace age_endline=. if age_endline>26 CHECK
...
* merge in block
* ---------------------------------------------
merge m:1 clusterid using "~/Dropbox/WBK-primary-analysis/Data/Untouched/washb-kenya-blind-tr.dta"
...
outsheet using "~/Dropbox/WBK-primary-analysis/Data/final/jade/midline-anthro.csv", replace comma 
save "~/Dropbox/WBK-primary-analysis/Data/final/jade/midline-anthro.dta", replace 
...
outsheet using "~/Dropbox/WBK-primary-analysis/Data/final/jade/endline-anthro.csv", replace comma 
save "~/Dropbox/WBK-primary-analysis/Data/final/jade/endline-anthro.dta", replace 
